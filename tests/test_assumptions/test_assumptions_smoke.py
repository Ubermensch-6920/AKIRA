...
def test_default_levers(sample_assumption_set: AssumptionSet) -> None:
    assert sample_assumption_set.stat.carvm.carvm_basis is StatCarvmBasis.AG35
    assert sample_assumption_set.stat.vm22.cte_level is CTELevel.CTE70
    assert sample_assumption_set.stat.vm22.reserve_component is Vm22Component.DR_SR_MAX
...
def test_validator_flags_bad_fas157_coc(sample_assumption_set: AssumptionSet) -> None:
    sample_assumption_set.us_gaap.fas157.cost_of_capital_rate = 0.99
    issues = validate_assumption_set(sample_assumption_set)
...
def test_validator_flags_bad_ebs_coc(sample_assumption_set: AssumptionSet) -> None:
    sample_assumption_set.ebs.technical_provisions.cost_of_capital_rate = 0.0
    issues = validate_assumption_set(sample_assumption_set)
    codes = [i.code for i in issues]

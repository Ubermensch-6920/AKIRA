"""Tests for the STAT-basis NAIC RBC calculation."""
...
from actuarial_model.bases.stat.rbc import RbcInput, calculate
...
@pytest.mark.parametrize(
    "framework", [Framework.BEL, Framework.EBS, Framework.LDTI, Framework.FAS157]
)
def test_non_stat_reserves_never_enter_rbc(
    framework: Framework, sample_assumption_set: AssumptionSet
):
    """RBC is a STAT measure: other bases' reserves are ignored, not summed."""
    output = calculate(
        RbcInput(
            assumption_set=sample_assumption_set,
            reserve_results=[_reserve(framework, 1_500_000.0)],
        )
    )
    components = output.capital_result.components
    assert components["statutory_reserve_base"] == 0.0
    assert components["reserve_framework_used"] is None
    assert output.capital_result.metadata.basis.value == "STAT"

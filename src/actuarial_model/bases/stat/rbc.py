"""
STAT — NAIC Risk-Based Capital (RBC) calculation.
...
RBC is a STAT-basis measure: reserves from other bases are ignored.
...
def _statutory_reserve_base(results: list[ReserveResult]) -> tuple[float, str | None]:
    ...
    STAT basis only: prefers VM-22, falls back to CARVM (pre-VM-22 runs).
    Reserves from other bases (US GAAP, LDTI, EBS) are never used.
    """
